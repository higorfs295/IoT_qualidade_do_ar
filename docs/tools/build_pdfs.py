...
def make_architecture_pdf():
    ...
    s += [Spacer(1, 4 * mm), p("ARQUITETURA DA SOLUÇÃO", "TitleQ"),
          ...
          table([["Componente", "Estado verificado"], ["Firmware HIL/físico/AWS", "Três builds; maior imagem usa 60,6% do slot OTA"], ["Placa alvo", "ESP-WROOM-32 DevKit 30P/USB-C; GPIOs cruzados"], ["Contrato", "11 testes Python, 6 Node e JSON Schema v1.1"], ["Backend/dashboard", "Ingestão, WebSocket, limites e teste de integração"], ["AWS", "IaC e consumidor testáveis; conta real não foi alterada"], ["CAD/EDA", "Roadmaps completos; arquivos nativos dependem das ferramentas e medições"]], [45*mm, 119*mm], 7.8)]
    ...
    s += [PageBreak(), p("8. Verificação e critérios de aceite", "H1Q"),
          table([["Gate", "Evidência exigida"], ["G0 — contrato", "testes Python e JavaScript; amostras válidas/inválidas"], ["G1 — software", "integração backend, HTTP/WS, limites e reconexão"], ["G2 — firmware", "build HIL/físico/AWS, serial, heap TLS e logs"], ["G3 — elétrica", "pinout/flash confirmados, ERC/DRC, trilhos e consumo"], ["G4 — sensores", "coleta lado a lado, repetibilidade, stale/erro e referência"], ["G5 — AWS", "mTLS, menor privilégio, reentrega/DLQ e revogação"], ["G6 — sistema", "ensaio prolongado, recuperação, deduplicação e retenção"], ["G7 — entrega", "documentação, BOM/CAD/EDA, versões e riscos"]], [40*mm,124*mm], 8),
          p("O que já foi automatizado", "H2Q"), bullet("11 testes Python, inclusive lote parcial do consumidor AWS, e 6 testes Node.js."), bullet("Autoteste do simulador e compilação do código Python/JavaScript."), bullet("Build PlatformIO dos ambientes esp32-hil, esp32-fisico e esp32-aws."), bullet("Teste integrado do backend com ingestão válida, listagem, métricas e conteúdo web."),
...
def make_slides_pdf():
    ...
    s += [PageBreak(), p("10. Verificação por gates", "SlideTitle"), table([["G0", "G1", "G2", "G3", "G4", "G5/G6"], ["contrato", "software", "firmware", "elétrica", "sensores", "AWS/sistema"], ["11 Py + 6 Node", "integração", "3 builds + heap", "pinout + ERC/DRC", "comparação", "mTLS/DLQ/soak"]], [doc.width/6]*6, 9.2), Spacer(1,8*mm), *slide_bullets(["Automatizado: 11 testes Python, 6 Node, autoteste do simulador e integração HTTP/WebSocket.", "Compilado: HIL, físico e AWS para esp32doit-devkit-v1/flash de 4 MB.", "Pendente físico/externo: flash e sensores reais, EasyEDA/SolidWorks nativos e sandbox AWS."])]
    s += [PageBreak(), p("11. Próximos marcos", "SlideTitle"), table([["1", "2", "3", "4", "5", "6"], ["bancada", "PCB A", "case A", "persistência", "AWS", "piloto"], ["pinout/sensores", "bring-up", "fluxo/térmica", "backup", "mTLS/DLQ", "go/no-go"]], [doc.width/6]*6, 9), Spacer(1,10*mm), p("Definição de pronto", "H1Q"), *slide_bullets(["Evidências dos gates anexadas e reproduzíveis.", "Riscos residuais registrados e aceitos.", "Documentação, código, BOM, CAD/EDA e versões sincronizados."]), Spacer(1,8*mm), p("Base pronta para validação física e cloud em sandbox.", "SubQ")]
    doc.build(s)
