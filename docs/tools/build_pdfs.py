...
def make_architecture_pdf():
    ...
    s += [Spacer(1, 4 * mm), p("ARQUITETURA DA SOLUÇÃO", "TitleQ"),
          ...
          table([["Componente", "Estado verificado"], ["Firmware HIL/físico/AWS", "Três builds; maior imagem usa 60,6% do slot OTA"], ["Placa alvo", "ESP-WROOM-32 DevKit 30P/USB-C; GPIOs cruzados"], ["Contrato", "11 testes Python, 9 Node e JSON Schema v1.1"], ["Stack local", "Compose, MQTT, persistência, API e PWA testados"], ["AWS", "IaC e consumidor testáveis; conta real não foi alterada"], ["CAD/EDA", "Roadmaps completos; arquivos nativos dependem das ferramentas e medições"]], [45*mm, 119*mm], 7.8)]
    ...
    s += [PageBreak(), p("8. Verificação e critérios de aceite", "H1Q"),
          ...
          p("O que já foi automatizado", "H2Q"), bullet("11 testes Python, inclusive lote parcial do consumidor AWS, e 9 testes Node.js."), bullet("Autoteste do simulador e compilação do código Python/JavaScript."), bullet("Build PlatformIO dos ambientes esp32-hil, esp32-fisico e esp32-aws."), bullet("Compose com MQTT, PWA, ingestão e restauração após reinício do backend."),
...
def make_slides_pdf():
    ...
    s += [PageBreak(), p("4. Firmware para ESP-WROOM-32", "SlideTitle"), *slide_bullets(["Três ambientes: HIL via Serial, físico com drivers e AWS com TLS/mTLS.", "Aquisição não bloqueante, checksum do PMS7003, algoritmo VOC e ADC1/GPIO34.", "ULID com época real, boot_id, sequência, timestamp após NTP e MQTT QoS 1.", "Partições 4 MB: dois slots OTA; payload/buffer de 896 B e gate de heap de 30 kB."]), Spacer(1,6*mm), table([["Build", "RAM", "Flash/slot OTA"], ["esp32-hil", "47.888 B · 14,6%", "787.625 B · 50,1%"], ["esp32-fisico", "47.996 B · 14,6%", "818.353 B · 52,0%"], ["esp32-aws", "49.024 B · 15,0%", "953.429 B · 60,6%"]], [75*mm,80*mm,80*mm], 9.5)]
    s += [PageBreak(), p("5. Dados e aplicação", "SlideTitle"), *slide_bullets(["Backend valida versão, ULID, faixas, sequência, tópico e consistência da qualidade.", "Deduplicação e séries são limitadas; snapshot atômico restaura o estado após reinício.", "PWA reage por WebSocket, reconecta, possui shell offline e sinaliza o caráter experimental.", "Próximo degrau para escala: TimescaleDB, retenção, usuários, backup e observabilidade externa."]), Spacer(1,6*mm), p("A stack Docker local sobe broker, gerador, backend e painel em um comando; a planilha calcula capacidade e armazenamento estimados.", "SubQ")]
    ...
    s += [PageBreak(), p("10. Verificação por gates", "SlideTitle"), table([["G0", "G1", "G2", "G3", "G4", "G5/G6"], ["contrato", "software", "firmware", "elétrica", "sensores", "AWS/sistema"], ["11 Py + 9 Node", "Compose + restore", "3 builds + heap", "pinout + ERC/DRC", "comparação", "mTLS/DLQ/soak"]], [doc.width/6]*6, 9.2), Spacer(1,8*mm), *slide_bullets(["Automatizado: 11 testes Python, 9 Node, simulador, API/PWA e persistência após restart.", "Compilado: HIL, físico e AWS para esp32doit-devkit-v1/flash de 4 MB.", "Pendente físico/externo: flash e sensores reais, EasyEDA/SolidWorks nativos e sandbox AWS."])]
